...
const createUser = async ({ name, surname, email, password, phone, birthday }: UserEntity): Promise<SuccessResponse | ErrorResponse> => {
	try {
		if (!(email && password && name && surname && phone && birthday)) return CreateErrorResponse(400, 'All input is required');
